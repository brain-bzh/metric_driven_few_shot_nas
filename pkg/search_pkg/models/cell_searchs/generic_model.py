...
class GenericNAS201Model(nn.Module):
    def __init__(
        ...
        self.n_edges = int(max_nodes*(max_nodes-1)/2)
        ...
        self.lastact = nn.Sequential(
            nn.BatchNorm2d(
                C_prev #TODO, affine=affine, track_running_stats=track_running_stats
            ),
        ...
        # algorithm related
        self.arch_parameters = torch.zeros(num_edge, len(search_space))
        self._arch_alphas = nn.Parameter(
    ...
    # just to get an iterable for the optimizer
    def arch_alphas(self):
        return [self._arch_alphas]

    def set_arch_alphas(self, alphas: list):
        self._arch_alphas = alphas[0]

    def get_weights(self):
        params = list(self._stem.parameters()) + list(self._cells.parameters())
        params += list(self.lastact.parameters()) + list(self.global_pooling.parameters())
    ...
    def genotype(self):
        ...
        theta = torch.softmax(self._arch_alphas[:self.n_edges,:], dim=-1) * self.arch_parameters
        genotypes = []
        final_arch = torch.zeros_like(self._arch_alphas[:self.n_edges,:])
